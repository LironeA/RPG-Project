...
namespace ToolExtensions
{
    public class RadialArrayCreator : EditorWindow
    {
        ...
        private int _instancesAmount = 1;
        ...
        {
            var window = (RadialArrayCreator) GetWindow(typeof(RadialArrayCreator));
        ...
        {
            var deltaAngle = 360f / (float) _instancesAmount;


            for (var r = 1; r <= _rings; r++)
            for (var i = 0; i < _instancesAmount * (_keepRingDensity ? r : 1); i++)
            {
                var angle = i * deltaAngle / (_keepRingDensity ? r : 1) + _ringOffset * r;
                var cosAngle = Mathf.Cos((angle + _angleOffset) * Mathf.Deg2Rad);
                var sinAngle = Mathf.Sin((angle + _angleOffset) * Mathf.Deg2Rad);
                ...
                if (_lookAtCenter) LookAtArrayOrigin(_lookAtAxleIndex, instantiatedObject);
        ...
        private GameObject ObjectInstantiate()
        {
            ...
            {
                instantiatedObject =
                    _parentToOrigin ? Instantiate(_objectToArray, _pivotPoint) : Instantiate(_objectToArray);
            }
            else
            {
                ...
                instantiatedObject = _parentToOrigin
                    ? (GameObject) PrefabUtility.InstantiatePrefab(_objecttoArrayPrefab, _pivotPoint)
            ...
            if (_avoidChildren && assetType == PrefabAssetType.NotAPrefab)
            {
                ...
                for (var i = cc; i >= 0; i--) DestroyImmediate(instantiatedObject.transform.GetChild(i).gameObject);
            }
        ...
        private void LookAtArrayOrigin(int index, GameObject io)
        {
            ...
            else if (index == 1) io.transform.Rotate(Vector3.right, 90, Space.Self);
        }
