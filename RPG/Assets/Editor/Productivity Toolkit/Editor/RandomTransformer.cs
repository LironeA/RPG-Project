...
namespace ToolExtensions
{
    ...
    public class RandomTransformer : EditorWindow
    {
        ...
        private RelationType _positionType = RelationType.Relative,
            _rotationType = RelationType.Absolute,
            _scaleType = RelationType.Absolute;

        public static GameObject[] _gameObjects;
        public static List<TransformElement> _transformElements = new List<TransformElement>();
        private List<Collider> _allcollidersInSelection = new List<Collider>();
        ...
        private Vector3 _maximumAxisScale;

        private bool _randomizePosition;
        private Vector3 _posMinRange, _posMaxRange;


        private bool _randomizerotation;
        private Vector3 _rotMinRange, _rotMaxRange;


        private bool _randomizeScale;
        private float _minUniformScale = 1;
        private float _maxUniformScale = 2;
        private bool _seperateAxisScale;
        private Vector3 _scaleMinRange = Vector3.one;
        private Vector3 _scaleMaxRange = Vector3.one;

        private GUIStyle _rangeOptions = new GUIStyle();
        private bool _showSelectedObjects;
        private Vector2 scrollPos;
        private string[] _searchOptions = new string[] {"Manual selection", "Contains name", "By tag"};
        private string[] _overlapOptions = new string[] {"Based on colliders", "Based on MeshRenderers"};
        private int _overlapOptionsIndex = 0;
        private bool _addChildren;
        private int _searchOptionsIndex = 0;
        ...
        {
            var window = (RandomTransformer) GetWindow(typeof(RandomTransformer));
        ...
        private void OnGUI()
        {
            ...
            if (_randomizeScale)
            {
                ...
                _seperateAxisScale =
                    EditorGUILayout.ToggleLeft("Separate Axis", _seperateAxisScale, GUILayout.MaxWidth(150));
                EditorGUILayout.EndHorizontal();
                ...
                {
                    _minUniformScale = EditorGUILayout.FloatField("Minimum scale range", _minUniformScale,
                        GUILayout.MaxWidth(250));
                    _maxUniformScale = EditorGUILayout.FloatField("Maximum scale range", _maxUniformScale,
                        GUILayout.MaxWidth(250));
                    EditorGUI.indentLevel--;
                }
            ...
            if (GUILayout.Button("Apply"))
            {
                ...
                foreach (var transformElement in _transformElements)
                {
                    ...
                    do
                    {
                        if (_randomizePosition)
                        {
                            var currentPosition = transformElement.originalPosition;
                            var offsetPosition = new Vector3(UnityEngine.Random.Range(_posMinRange.x, _posMaxRange.x),
                                UnityEngine.Random.Range(_posMinRange.y, _posMaxRange.y),
                                UnityEngine.Random.Range(_posMinRange.z, _posMaxRange.z));
                            if (_positionType == RelationType.Absolute)
                                ...
                                transformElement.TheGameObject.transform.localPosition =
                                    currentPosition + offsetPosition;
                        }

                        if (_randomizerotation)
                        {
                            var currentRotation = transformElement.TheGameObject.transform.localRotation;
                            var offsetRotation =
                                Quaternion.Euler(UnityEngine.Random.Range(_rotMinRange.x, _rotMaxRange.x),
                                    UnityEngine.Random.Range(_rotMinRange.y, _rotMaxRange.y),
                                    UnityEngine.Random.Range(_rotMinRange.z, _rotMaxRange.z));
                            ...
                            if (_rotationType == RelationType.Absolute)
                                ...
                                transformElement.TheGameObject.transform.localRotation =
                                    currentRotation * offsetRotation;
                        }

                        if (_randomizeScale)
                        {
                            var currentScale = transformElement.TheGameObject.transform.localScale;
                            Vector3 offsetScale;
                            if (_seperateAxisScale)
                            {
                                offsetScale = new Vector3(UnityEngine.Random.Range(_scaleMinRange.x, _scaleMaxRange.x),
                                    UnityEngine.Random.Range(_scaleMinRange.y, _scaleMaxRange.y),
                                    UnityEngine.Random.Range(_scaleMinRange.z, _scaleMaxRange.z));
                            }
                            else
                            {
                                var randomRangeValue = UnityEngine.Random.Range(_minUniformScale, _maxUniformScale);
                            ...
                            switch (_scaleType)
                            {
                                case RelationType.Absolute:
                                    transformElement.TheGameObject.transform.localScale =
                                        Vector3.Scale(Vector3.one, offsetScale);
                                    break;
                                case RelationType.Relative:
                                    transformElement.TheGameObject.transform.localScale =
                                        Vector3.Scale(transformElement.originalScale, offsetScale);
                                    break;
                                default:
                                    break;
                            }
                        ...
                        if (_overlapOptionsIndex == 0)
                        {
                            if (_addChildren)
                                collisionResult =
                                    CheckIntercollisionChildren(transformElement.TheGameObject, typeof(Collider));
                            else
                                collisionResult =
                                    CheckIntercollision(transformElement.TheGameObject.GetComponent<Collider>());
                        }
                        else if (_overlapOptionsIndex == 1)
                        {
                            if (_addChildren)
                                collisionResult = CheckIntercollisionChildren(transformElement.TheGameObject,
                                    typeof(MeshRenderer));
                            else
                                collisionResult =
                                    CheckIntercollision(transformElement.TheGameObject.GetComponent<MeshRenderer>());
                        }
        ...
        {
            var bounds = new Bounds(); // the bounds of the single object
            if (type == typeof(Collider))
            {
                var allColliders = theGameObject.GetComponentsInChildren<Collider>();
            ...
            {
                var allMeshRenderers = theGameObject.GetComponentsInChildren<MeshRenderer>();
                foreach (var mr in allMeshRenderers)
                {
                    if (bounds.center == Vector3.zero) bounds = mr.bounds;
            ...
            foreach (var go in _transformElements)
            {
                ...
                var boundsCompare = new Bounds();
                if (type == typeof(Collider))
                {
                    var allColliders = go.TheGameObject.GetComponentsInChildren<Collider>();
                    foreach (var collider in allColliders)
                ...
                {
                    var allMeshRenderers = go.TheGameObject.GetComponentsInChildren<MeshRenderer>();
                    foreach (var mr in allMeshRenderers)
                        if (boundsCompare.center == Vector3.zero)
                ...
                if (bounds.Intersects(boundsCompare)) return true;
            }
        ...
        {
            var undoableObjects = new Transform[_transformElements.Count];
            for (var i = 0; i < _transformElements.Count; i++)
        ...
        private void SetRangesPerAxis(float minValue, float maxValue, Axis axis, ref Vector3 axisMinValues,
            ref Vector3 axisMaxValues)
        {
            if (axis == Axis.X)
            {
        ...
        private bool CheckIntercollision(Collider currentCollider)
        {
            foreach (var colliderInSelection in _allcollidersInSelection)
                if (colliderInSelection != currentCollider &&
                    currentCollider.bounds.Intersects(colliderInSelection.bounds))
                {
                    Debug.Log(
                        $"Collision between {currentCollider.gameObject.name} and {colliderInSelection.gameObject.name}. Replacing object");
                    return true;
                }
        ...
        private bool CheckIntercollision(MeshRenderer currentMeshRenderer)
        {
            foreach (var mrInSelection in _allMeshRenderers)
                ...
                {
                    Debug.Log(
                        $"MeshRenderer between {currentMeshRenderer.gameObject.name} and {mrInSelection.gameObject.name}. Replacing object");
                    return true;
                }
