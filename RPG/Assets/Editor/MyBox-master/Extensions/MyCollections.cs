...
namespace MyBox
{
    public static class MyCollections
    {
        ...
        {
            foreach (var element in source) func(element);
            return source;
        }
        ...
        {
            for (var i = 0; i < source.Count; ++i) source[i] = valueFactory(i);
            return source;
        }
            ...
            int sampleNumber)
        {
            if (sampleNumber > source.Count)
                throw new ArgumentOutOfRangeException(
                    "Cannot sample more elements than what the source collection contains");
            var results = new T[sampleNumber];
            var resultIndex = 0;
            for (var i = 0; i < source.Count && resultIndex < sampleNumber; ++i)
            {
                var probability = (double) (sampleNumber - resultIndex)
                                  / (source.Count - i);
                if (MyCommonConstants.SystemRandom.NextDouble() < probability)
                {
                    results[resultIndex] = source[i];
                    ++resultIndex;
                }
            }
