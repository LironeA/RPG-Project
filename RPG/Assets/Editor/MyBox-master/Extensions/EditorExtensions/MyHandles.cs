...
namespace MyBox.EditorTools
{
    public static class MyHandles
    {
        ...
        /// </summary>
        public static void DrawDirectionalLine(Vector3 fromPos, Vector3 toPos, float screenSpaceSize = 3,
            float arrowsDensity = .5f)
        {
        ...
        /// </summary>
        public static void DrawTinyArrow(Vector3 position, Vector3 direction, float headLength = 0.25f,
            float headAngle = 20.0f)
        {
            var lookRotation = Quaternion.LookRotation(direction);
            var rightVector = new Vector3(0, 0, 1);
            var right = lookRotation * Quaternion.Euler(0, 180 + headAngle, 0) * rightVector;
            var left = lookRotation * Quaternion.Euler(0, 180 - headAngle, 0) * rightVector;
