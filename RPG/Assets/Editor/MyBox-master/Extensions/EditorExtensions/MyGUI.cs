...
namespace MyBox.EditorTools
{
    public static class MyGUI
    {
        ...
        {
            if (PlusButton()) return property.NewElement();
            ...
            string externalImportFolder = null) where T : Object
        {
            ...
            var dragEvent = currentEvent.type == EventType.DragUpdated || currentEvent.type == EventType.DragPerform;
            if (!dragEvent) return null;
            var overDropArea = dropArea.Contains(currentEvent.mousePosition);
            ...
            var anyExternal = DragAndDrop.paths.Length > 0 &&
            ...
            if (allowExternal && anyExternal)
            {
                ...
                foreach (var externalPath in DragAndDrop.paths)
                {
            ...
            else
            {
                foreach (var dragged in DragAndDrop.objectReferences)
                {
                    var validObject = dragged as T ??
                                      AssetDatabase.LoadAssetAtPath<T>(AssetDatabase.GetAssetPath(dragged));
        ...
        public static string BrowsFileLabel(string name, float labelWidth, string path, string extension)
        {
            ...
            if (GUILayout.Button("Browse")) filepath = EditorUtility.OpenFilePanel(name, path, extension);
        ...
        {
            EditorGUILayout.BeginHorizontal();
            var filepath = EditorGUILayout.TextField(name, path, GUILayout.MaxWidth(labelWidth));
        ...
        {
            var toolbar = Enum.GetNames(selected.GetType());
            var values = Enum.GetValues(selected.GetType());

            for (var i = 0; i < toolbar.Length; i++)
            {
                var toolName = toolbar[i];
            ...
            {
                if (selected.ToString() == values.GetValue(selectedIndex).ToString()) break;
        ...
        public static string[] ArrayFoldout(string label, string[] array, ref bool foldout)
        {
            ...
            if (foldout)
            {
                ...
                EditorGUILayout.Space();
                EditorGUILayout.BeginVertical();
                var arraySize = EditorGUILayout.IntField("Size", array.Length);
        ...
        public static bool EyeButton(bool on)
        {
            ...
            return GUILayout.Button(on ? _eyeOpenContent : _eyeClosedContent, EditorStyles.toolbarButton,
                GUILayout.Width(20));
        }
