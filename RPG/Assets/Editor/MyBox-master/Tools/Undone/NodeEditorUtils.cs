...
namespace MyBox.Internal
{
    public static class NodeEditorUtils
    {
        ...
        public static void DrawNodeCurve(Rect start, Vector3 endPosition)
        {
            ...
            for (var i = 0; i < 3; i++)
                // Draw a shadow
                Handles.DrawBezier(startPosition, endPosition, startTangent, endTangent, shadowColor, null,
                    (i + 1) * 5);
        ...
        {
            var position = GUILayoutUtility.GetRect(GUIContent.none, SplitterStyle, GUILayout.Height(thickness));
        ...
        {
            var position = GUILayoutUtility.GetRect(GUIContent.none, splitterStyle, GUILayout.Width(thickness));
