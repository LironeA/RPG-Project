...
namespace MyBox.Internal
{
    ...
    internal class Bundle<T>
    {
        ...
        {
            var haveDataWithSameKey = bundleData.ContainsKey(dataKey);

            if (haveDataWithSameKey && overrideIfExists)
                bundleData[dataKey] = data;
            else if (!haveDataWithSameKey) bundleData.Add(dataKey, data);
        }

        internal void AddData(KeyValuePair<string, T> keyValuePair, bool overrideIfExists = true)
        {
            var haveDataWithSameKey = bundleData.ContainsKey(keyValuePair.Key);
        ...
        {
            foreach (var data in bundle) AddData(data, overrideIfExists);
        }
