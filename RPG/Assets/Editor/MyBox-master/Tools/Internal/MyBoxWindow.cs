    ...
    public class MyBoxWindow : EditorWindow
    {
        ...
        private static void CheckForUpdates()
        {
            ...
            MyBoxUtilities.GetMyBoxLatestVersionAsync(version =>
            {
                ...
                {
                    var versions = "Installed version: " + _installedVersion.AsSting + ". Latest version: " +
                                   _latestVersion.AsSting;
        ...
        private void OnGUI()
        {
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                if (GUILayout.Button("  Github Page ↗", _buttonStyle, GUILayout.Width(buttonWidth),
                        GUILayout.Height(buttonHeight)))
                    Application.OpenURL("https://github.com/Deadcows/MyBox");

                if (GUILayout.Button("  Attributes ↗", _buttonStyle, GUILayout.Width(buttonWidth),
                        GUILayout.Height(buttonHeight)))
                    Application.OpenURL("https://github.com/Deadcows/MyBox/wiki/Attributes");

                if (GUILayout.Button("  Extensions ↗", _buttonStyle, GUILayout.Width(buttonWidth),
                        GUILayout.Height(buttonHeight)))
                    Application.OpenURL("https://github.com/Deadcows/MyBox/tree/master/Extensions");

                if (GUILayout.Button("  Tools, Features ↗", _buttonStyle, GUILayout.Width(buttonWidth),
                        GUILayout.Height(buttonHeight)))
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                using (new EditorGUILayout.VerticalScope())
                {
                    using (new EditorGUILayout.HorizontalScope())
                    {
                        ...
                        MyBoxSettings.CheckForUpdates =
                            EditorGUILayout.Toggle("Check for Updates: ", MyBoxSettings.CheckForUpdates);
                        GUILayout.FlexibleSpace();
                    }

                    using (new EditorGUILayout.HorizontalScope())
                    {
                        var label = new GUIContent("AutoSave on Play: ",
                            "Save changes in opened scenes before Playmode. " +
                    ...
                    using (new EditorGUILayout.HorizontalScope())
                    {
                        var label = new GUIContent("Clean Empty Folders: ",
                            "Delete empty folders in project on Save. " +
                            "\nIt handles VCS issue with .meta files for empty folders");
                        EditorGUILayout.Space(leftOffset);
                        MyBoxSettings.CleanEmptyDirectoriesFeature =
                            EditorGUILayout.Toggle(label, MyBoxSettings.CleanEmptyDirectoriesFeature);
                        GUILayout.FlexibleSpace();
                    }
                }
                ...
                using (new EditorGUILayout.VerticalScope())
                {
                    ...
                    using (new EditorGUILayout.HorizontalScope())
                    {
                        var label = new GUIContent("Prepare on Playmode: ",
                            "Allows to use IPrepare interface with Prepare() method called automatically." +
                            "\nSlightly increases project Save time.");
                        MyBoxSettings.PrepareOnPlaymode =
                            EditorGUILayout.Toggle(label, MyBoxSettings.PrepareOnPlaymode);
                    ...
                    using (new EditorGUILayout.HorizontalScope())
                    {
                        var label = new GUIContent("SO processing: ",
                            "Allows [AutoProperty] and [MustBeAssigned] Attributes to work with Scriptable Objects." +
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                if (GUILayout.Button(updateOrInstall + " UPM version", _buttonStyle, GUILayout.Width(buttonWidth),
                        GUILayout.Height(buttonHeight)))
                {
                ...
                if (GUILayout.Button("  How to Update ↗", _buttonStyle, GUILayout.Width(buttonWidth),
                        GUILayout.Height(buttonHeight)))
                    Application.OpenURL("https://github.com/Deadcows/MyBox/wiki/Installation");

                if (GUILayout.Button("  Releases ↗", _buttonStyle, GUILayout.Width(buttonWidth),
                        GUILayout.Height(buttonHeight)))
                    Application.OpenURL("https://github.com/Deadcows/MyBox/releases");

                if (GUILayout.Button("  Changelog ↗", _buttonStyle, GUILayout.Width(buttonWidth),
                        GUILayout.Height(buttonHeight)))
            ...
            EditorGUILayout.Space();
            EditorGUILayout.LabelField("MyGUI.EditorIcons References + with black color tint",
        ...
        {
            var width = 24;
            var height = (int) EditorGUIUtility.singleLineHeight;
        ...
        private Texture2D SponsorButton => _sponsorButton != null
            ? _sponsorButton
