...
namespace MyBox.Internal
{
    public class SerializedPropertyViewer : EditorWindow
    {
        ...
        private static void ContextMenuItem(MenuCommand command)
        {
        ...
        private void OnGUI()
        {
            ...
            var searchString = EditorGUILayout.TextField("Search:", _searchString);
            ...
            foreach (var property in _propertiesData)
            {
                ...
                if (property.ObjectId > 0) GUILayout.BeginHorizontal();

                EditorGUILayout.SelectableLabel(property.Info, _richTextStyle, GUILayout.Height(20));
                if (property.ObjectId > 0)
                {
                    if (GUILayout.Button(">Ping>", GUILayout.Width(50))) Selection.activeInstanceID = property.ObjectId;
            ...
            if (GUILayout.Button("Copy To Clipboard"))
            {
                ...
                var paddingHash = new Dictionary<int, string>();
                var padding = "";
                for (var i = 0; i < 40; i++)
                {
                ...
                foreach (var line in _propertiesData)
                {
        ...
        private void CollectProperties()
        {
            ...
            while (iterator.Next(true)) LogPropertyData(iterator);
        }
        ...
        private void LogPropertyData(SerializedProperty property)
        {
            ...
            if (property.isArray)
                _descriptionBuilder.Append("<color=maroon>").Append(ProcessSelection("[Array]")).Append("</color> ");
            _descriptionBuilder.Append("<color=blue>").Append(ProcessSelection(property.type)).Append("</color>");

            _descriptionBuilder.Append(ProcessSelection(", Name:")).Append("<color=green>")
                .Append(ProcessSelection(property.name))
                .Append("</color>");
            ...
            var description = _descriptionBuilder.ToString();


            var isObject = property.propertyType == SerializedPropertyType.ObjectReference &&
                           property.objectReferenceValue != null;
            var propertyId = isObject ? property.objectReferenceValue.GetInstanceID() : 0;
