...
namespace MyBox.Internal
{
    ...
    public class CollapsableEventDrawer : UnityEventDrawer
    {
        ...
        private ReorderableList GetList(SerializedProperty property)
        {
            ...
            if (_lists.ContainsKey(state)) return _lists[state];

            var list = GetReorderableListFromState(state);
            list.draggable = true;
            _lists[state] = list;
            return list;
        }
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            var elements = property.FindPropertyRelative(CALLS_PROPERTY_PATH);
            var heightTemp = position.height;
        ...
        private static ReorderableList GetReorderableListFromState(State state)
        {
            if (_stateReorderableListFieldInfo == null)
                _stateReorderableListFieldInfo =
                    typeof(State).GetField(REORDERABLE_LIST_FIELD_NAME, NON_PULIC_INSTANCE_FLAGS);
