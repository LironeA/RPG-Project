...
namespace MyBox
{
    public class ColliderGizmo : MonoBehaviour
    {
        ...
        public void Refresh()
        {
            ...
            var colliders2d = IncludeChildColliders
        ...
        private void DrawEdgeCollider2D(EdgeCollider2D coll)
        {
            ...
            var first = true;
            for (var i = 0; i < coll.points.Length; i++)
            {
                var collPoint = coll.points[i];
                var pos = new Vector3(collPoint.x * lossyScale.x, collPoint.y * lossyScale.y, 0);
                var rotated = target.rotation * pos;
        ...
        private void DrawCircleCollider2D(CircleCollider2D coll)
        {
            ...
            DrawColliderGizmo(target.position + new Vector3(offset.x, offset.y, 0.0f),
                coll.radius * Mathf.Max(scale.x, scale.y));
        }
...
namespace MyBox.Internal
{
    ...
    public class ColliderGizmoEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            var collidersCountCheck = CollidersCount();
            var collidersCountChanged = collidersCountCheck != _collidersCount;
