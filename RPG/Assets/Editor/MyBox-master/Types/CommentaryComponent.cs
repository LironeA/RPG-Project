        ...
        public override void OnInspectorGUI()
        {
            ...
            if (!_editMode && _commentary.Entries.Length > 0)
            {
                ...
                foreach (var entry in _commentary.Entries)
                {
                    ...
                    if (GUILayoutUtility.GetLastRect().Contains(e.mousePosition) && e.button == 0 && e.isMouse)
                        _editMode = true;
                }
            ...
            if (_editMode || _commentary.Entries.Length == 0)
            {
                for (var i = 0; i < _commentary.Entries.Length; i++)
                {
                    ...
                    using (new EditorGUILayout.HorizontalScope())
                    {
                        ...
                        {
                            GUILayout.Space(4);
                            if (GUILayout.Button(GetIcon(entry.Type), EditorStyles.helpBox, GUILayout.Width(40),
                                    GUILayout.Height(36))) _commentary.Entries[i].Type = NextType(entry.Type);
                        ...
                        _commentary.Entries[i].EditorCommentary =
                            GUILayout.TextArea(entry.EditorCommentary, EditorStyles.helpBox);
                    }
                }
                ...
                {
                    if (_commentary.Entries.Length > 0 && GUILayout.Button(MyGUI.Characters.Check, GUILayout.Width(40)))
                        _editMode = false;
