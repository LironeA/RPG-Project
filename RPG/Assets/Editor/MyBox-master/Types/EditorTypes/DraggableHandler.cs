...
namespace MyBox.EditorTools
{
    ...
    public class DraggableHandler
    {
        ...
        {
            return DraggableHandle(position, .5f, Quaternion.identity, Handles.SphereHandleCap, Color.green,
                out result);
        }

        public static Vector3 DraggableHandle(Vector3 position, float handleSize, Quaternion rotation,
            Handles.CapFunction capFunc, Color colorSelected,
            out DragHandleResult result, bool allowRightButton = false)
        {
            result = DragHandleResult.None;

            var id = GUIUtility.GetControlID(DragHandleHash, FocusType.Passive);
            LastDragHandleId = id;

            var screenPosition = Handles.matrix.MultiplyPoint(position);
            var cachedMatrix = Handles.matrix;

            var leftMouseUsed = Event.current.button == 0;
            var rightMouseUsed = Event.current.button == 1;
            var validControl = leftMouseUsed || rightMouseUsed && allowRightButton;
            ...
            switch (Event.current.GetTypeForControl(id))
            {
                ...
                case EventType.MouseUp:
                    if (GUIUtility.hotControl == id && validControl)
                    {
                        ...
                        {
                            var doubleClick = _dragHandleClickId == id &&
                                              Time.realtimeSinceStartup - _dragHandleClickTime <
                                              DragHandleDoubleClickInterval;
                            ...
                            if (!doubleClick)
                                ...
                                result = leftMouseUsed
                                    ? DragHandleResult.LMBDoubleClick
                                    : DragHandleResult.RMBDoubleClick;
                        }
                    }
                ...
                case EventType.MouseDrag:
                    if (GUIUtility.hotControl == id && validControl)
                    {
                        ...
                        var position2 =
                            Camera.current.WorldToScreenPoint(Handles.matrix.MultiplyPoint(_dragHandleWorldStart))
    ...
    public static class DraggableHandlerExtensions
    {
        ...
        {
            return result == DraggableHandler.DragHandleResult.LMBPress ||
                   result == DraggableHandler.DragHandleResult.RMBPress;
        }

        public static bool IsMouseRelease(this DraggableHandler.DragHandleResult result)
        {
            return result == DraggableHandler.DragHandleResult.LMBRelease ||
                   result == DraggableHandler.DragHandleResult.RMBRelease;
        }

        public static bool IsMouseDrag(this DraggableHandler.DragHandleResult result)
        {
            return result == DraggableHandler.DragHandleResult.LMBDrag ||
                   result == DraggableHandler.DragHandleResult.RMBDrag;
        }
    }
}
#endif