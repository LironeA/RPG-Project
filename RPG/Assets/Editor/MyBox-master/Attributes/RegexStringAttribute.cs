...
namespace MyBox
{
    ...
    public class RegexStringAttribute : PropertyAttribute
    {
        ...
        public RegexStringAttribute(string regex, RegexStringMode mode = RegexStringMode.Match,
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            if (property.propertyType != SerializedPropertyType.String)
            {
                ...
                EditorGUI.LabelField(position,
                    new GUIContent("", "[RegexStringAttribute] used with non-string property"));
            }
            else
            {
                var regex = (RegexStringAttribute) attribute;
                var mode = regex.AttributeMode;
                var ifMatch = mode == RegexStringMode.WarningIfMatch;
                var ifNotMatch = mode == RegexStringMode.WarningIfNotMatch;
                var anyMatching = regex.Regex.IsMatch(property.stringValue);
                var warn = ifMatch && anyMatching || ifNotMatch && !anyMatching;
                ...
                void OnReplace()
                {
                    property.stringValue = regex.Regex.Replace(property.stringValue, "");
                }

                void OnKeepMatching()
                {
                    property.stringValue = regex.Regex.KeepMatching(property.stringValue);
                }
